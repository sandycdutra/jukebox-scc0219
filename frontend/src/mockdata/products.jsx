// frontend/mockdata/products.jsx
const products = [
    {
        id: 1,
        type: 'vinyl',
        title: 'Midnights (The Til Dawn Edition)',
        artist: 'Taylor Swift',
        price: 129.90,
        image: 'https://i.scdn.co/image/ab67616d0000b273fa747621a53c8e2cc436dee0', // Imagem para a ProductCard
        // NOVOS CAMPOS PARA A PÁGINA DE DETALHES:
        description: 'Midnights (The Til Dawn Edition) é a versão expandida do aclamado décimo álbum de estúdio da cantora e compositora americana Taylor Swift. Lançado em 2022, o álbum explora as noites e os pensamentos que surgem à meia-noite, com letras introspectivas e uma sonoridade pop mais madura. Esta edição especial inclui faixas bônus e é um item essencial para os fãs de Taylor Swift.',
        images: [
            'https://i.scdn.co/image/ab67616d0000b273fa747621a53c8e2cc436dee0', // Imagem principal (pode ser a mesma da listagem ou uma maior)
            'https://media.pitchfork.com/photos/635a96753a808ce453664d9f/1:1/w_320/Taylor-Swift-Midnights.jpg', // Exemplo de miniatura 1
            'https://m.media-amazon.com/images/I/71u9sJm9qEL._AC_UF894,1000_QL80_.jpg', // Exemplo de miniatura 2
            // Adicione mais URLs de imagens se tiver
        ],
    },
    {
        id: 2,
        type: 'vinyl',
        title: 'Renaissance',
        artist: 'Beyoncé',
        price: 119.90,
        image: 'https://upload.wikimedia.org/wikipedia/pt/thumb/9/9d/Renaissance_-_Beyonc%C3%A9.png/250px-Renaissance_-_Beyonc%C3%A9.png',
        description: 'Renaissance é o sétimo álbum de estúdio da superestrela global Beyoncé. Lançado em 2022, o álbum é uma celebração da cultura club e da música disco, com faixas que homenageiam os pioneiros do gênero. É um trabalho vibrante e audacioso que convida à dança e à libertação, recebendo aclamação crítica por sua inovação e produção impecável.',
        images: [
            'https://upload.wikimedia.org/wikipedia/pt/thumb/9/9d/Renaissance_-_Beyonc%C3%A9.png/250px-Renaissance_-_Beyonc%C3%A9.png',
            'https://m.media-amazon.com/images/I/81577H1LdGL._UF894,1000_QL80_.jpg',
        ]
    },
    {
        id: 3,
        type: 'cd',
        title: 'GUTS',
        artist: 'Olivia Rodrigo',
        price: 49.90,
        image: 'https://cdn-images.dzcdn.net/images/cover/4bb79214365c0049e031f5e2caae4752/0x1900-000000-80-0-0.jpg',
        description: 'GUTS é o segundo álbum de estúdio da cantora e compositora americana Olivia Rodrigo. Lançado em 2023, o álbum aprofunda os temas de angústia adolescente e desafios da juventude com uma energia mais rock e letras ainda mais afiadas. Inclui os sucessos "vampire" e "bad idea right?", consolidando Rodrigo como uma das vozes mais autênticas de sua geração.',
        images: [
            'https://cdn-images.dzcdn.net/images/cover/4bb79214365c0049e031f5e2caae4752/0x1900-000000-80-0-0.jpg',
            'https://i.ebayimg.com/images/g/V3oAAOSwX2Rk~wF~/s-l1600.jpg',
        ]
    },
    // Continue com os outros produtos ou adicione description e images para eles se quiser
    // ... (o restante dos seus produtos)
    {
        id: 4,
        type: 'vinyl',
        title: 'SOS',
        artist: 'SZA',
        price: 109.90,
        image: 'https://upload.wikimedia.org/wikipedia/pt/c/c8/SZA_-_SOS.png',
        description: 'SOS é o segundo álbum de estúdio da cantora americana SZA, lançado em 2022. O álbum de R&B contemporâneo explora as complexidades dos relacionamentos, o crescimento pessoal e a autoaceitação, com uma mistura de gêneros que vão do pop-punk ao gospel. Foi aclamado pela crítica e rendeu diversos hits, solidificando a posição de SZA como uma força inovadora na música atual.',
        images: [
            'https://upload.wikimedia.org/wikipedia/pt/c/c8/SZA_-_SOS.png',
            'https://m.media-amazon.com/images/I/61Nl5tG9t2L._UF894,1000_QL80_.jpg',
        ]
    },
    {
        id: 5,
        type: 'vinyl',
        title: 'The Highlights',
        artist: 'The Weeknd',
        price: 99.90,
        image: 'https://universalmusic.vtexassets.com/arquivos/ids/188179/vinil-the-weeknd-the-highlights-2lp-importado-vinil-the-weeknd-the-highlights-2lp-00602435931975-00060243593197.jpg?v=638622776291070000',
        description: 'The Highlights é a primeira coletânea de sucessos do cantor canadense The Weeknd, lançada em 2021. Reúne os maiores êxitos de sua carreira, desde "Blinding Lights" e "Heartless" até faixas de seus primeiros trabalhos. É um álbum essencial para qualquer um que queira explorar a evolução sonora de um dos artistas mais influentes da década.',
        images: [
            'https://universalmusic.vtexassets.com/arquivos/ids/188179/vinil-the-weeknd-the-highlights-2lp-importado-vinil-the-weeknd-the-highlights-2lp-00602435931975-00060243593197.jpg?v=638622776291070000',
            'https://m.media-amazon.com/images/I/71N-Wf2iGHL._UF894,1000_QL80_.jpg',
        ]
    },
    {
        id: 6,
        type: 'cd',
        title: 'UTOPIA',
        artist: 'Travis Scott',
        price: 59.90,
        image: 'https://cdn-images.dzcdn.net/images/cover/6c91e64b7157f1332a4f6b0de9e4c714/0x1900-000000-80-0-0.jpg',
        description: 'UTOPIA é o quarto álbum de estúdio do rapper americano Travis Scott. Lançado em 2023, o álbum é uma jornada sonora complexa e experimental, misturando elementos de trap, psicodelia e rock. Com participações de diversos artistas renomados, UTOPIA é um projeto ambicioso que solidifica a estética sonora única de Scott.',
        images: [
            'https://cdn-images.dzcdn.net/images/cover/6c91e64b7157f1332a4f6b0de9e4c714/0x1900-000000-80-0-0.jpg',
            'https://m.media-amazon.com/images/I/71F2b+j2+aL._UF894,1000_QL80_.jpg',
        ]
    },
    {
        id: 7,
        type: 'vinyl',
        title: 'Endless Summer Vacation',
        artist: 'Miley Cyrus',
        price: 115.90,
        image: 'https://upload.wikimedia.org/wikipedia/pt/c/cd/Endless_Summer_Vacation.jpg',
        description: 'Endless Summer Vacation é o oitavo álbum de estúdio da cantora e compositora americana Miley Cyrus. Lançado em 2023, o álbum celebra a autoaceitação e a liberdade, com canções que transitam entre o pop, rock e baladas introspectivas. Inclui o hit global "Flowers" e é um testemunho da evolução artística de Cyrus.',
        images: [
            'https://upload.wikimedia.org/wikipedia/pt/c/cd/Endless_Summer_Vacation.jpg',
            'https://m.media-amazon.com/images/I/71lC5p1wP+L._UF894,1000_QL80_.jpg',
        ]
    },
    {
        id: 8,
        type: 'vinyl',
        title: 'Un Verano Sin Ti',
        artist: 'Bad Bunny',
        price: 125.90,
        image: 'https://upload.wikimedia.org/wikipedia/pt/6/60/Bad_Bunny_-_Un_Verano_Sin_Ti.png',
        description: 'Un Verano Sin Ti é o quarto álbum de estúdio do artista porto-riquenho Bad Bunny. Lançado em 2022, o álbum é uma mistura eclética de reggaeton, dembow e indie pop, com letras que abordam amor, desilusão e a cultura caribenha. É um fenômeno global que quebrou recordes e consolidou Bad Bunny como um dos artistas mais populares do mundo.',
        images: [
            'https://upload.wikimedia.org/wikipedia/pt/6/60/Bad_Bunny_-_Un_Verano_Sin_Ti.png',
            'https://m.media-amazon.com/images/I/81k32V2Qk6L._UF894,1000_QL80_.jpg',
        ]
    },
    {
        id: 9,
        type: 'cd',
        title: 'Harry\'s House',
        artist: 'Harry Styles',
        price: 79.90,
        image: 'https://upload.wikimedia.org/wikipedia/pt/d/d5/Harry_Styles_-_Harry%27s_House.png',
        description: 'Harry\'s House é o terceiro álbum de estúdio do cantor e compositor britânico Harry Styles. Lançado em 2022, o álbum é uma coleção de canções pop-funk e synth-pop com toques de soul e R&B. É um trabalho íntimo e divertido que explora temas de amor, casa e auto-descoberta, elogiado por sua produção elegante e letras cativantes.',
        images: [
            'https://upload.wikimedia.org/wikipedia/pt/d/d5/Harry_Styles_-_Harry%27s_House.png',
            'https://m.media-amazon.com/images/I/71wLp9V5a-L._UF894,1000_QL80_.jpg',
        ]
    },
    {
        id: 10,
        type: 'vinyl',
        title: 'Beethoven: The 9 Symphonies',
        artist: 'Berliner Philharmoniker & Herbert von Karajan',
        price: 299.90,
        image: 'https://m.media-amazon.com/images/I/81k2atuVA5L._UF1000,1000_QL80_.jpg',
        description: 'Beethoven: The 9 Symphonies é uma gravação clássica das nove sinfonias de Ludwig van Beethoven pela Filarmônica de Berlim, regida por Herbert von Karajan. Este conjunto é considerado uma das interpretações definitivas das obras-primas de Beethoven, oferecendo uma experiência sonora rica e imersiva que é essencial para amantes da música clássica e audiófilos.',
        images: [
            'https://m.media-amazon.com/images/I/81k2atuVA5L._UF1000,1000_QL80_.jpg',
            'https://m.media-amazon.com/images/I/81k2atuVA5L._AC_SL1500_.jpg',
        ]
    },
    {
        id: 11,
        type: 'cd',
        title: 'Planet Her',
        artist: 'Doja Cat',
        price: 69.90,
        image: 'https://m.media-amazon.com/images/I/81YJQEQPmJL._UF1000,1000_QL80_.jpg',
        description: 'Planet Her é o terceiro álbum de estúdio da rapper e cantora americana Doja Cat. Lançado em 2021, o álbum é uma fusão de pop, R&B, hip hop e toques de afrobeats, com letras que exploram temas de sexualidade, feminilidade e relacionamentos. Com hits como "Kiss Me More" e "Woman", o álbum demonstra a versatilidade e o talento de Doja Cat.',
        images: [
            'https://m.media-amazon.com/images/I/81YJQEQPmJL._UF1000,1000_QL80_.jpg',
            'https://m.media-amazon.com/images/I/61Nl5tG9t2L._UF894,1000_QL80_.jpg',
        ]
    },
    {
        id: 12,
        type: 'vinyl',
        title: 'MAYHEM', // O nome do álbum é "Chromatica", não "Mayhem"
        artist: 'Lady Gaga',
        price: 139.90,
        image: 'https://upload.wikimedia.org/wikipedia/pt/0/0a/Lady_Gaga_-_Mayhem.jpg',
        description: 'Chromatica é o sexto álbum de estúdio da cantora americana Lady Gaga. Lançado em 2020, o álbum é um retorno às raízes dance-pop de Gaga, com uma sonoridade eletrônica inspirada nos anos 90 e temas de cura e superação de traumas. Inclui os singles "Stupid Love" e "Rain on Me", e é uma ode à resiliência e à alegria da música eletrônica.',
        images: [
            'https://upload.wikimedia.org/wikipedia/pt/0/0a/Lady_Gaga_-_Mayhem.jpg', // Usando a imagem que você forneceu
            'https://m.media-amazon.com/images/I/81nZ5d2hG+L._UF894,1000_QL80_.jpg',
        ]
    }
];

export default products;
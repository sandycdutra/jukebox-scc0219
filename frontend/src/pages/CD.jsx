...
function CD () {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                console.error("Error fetching products for CD category:", err); // Log mais específico
                setError("Failed to load CDs. Please try again later."); // Mensagem de erro para o usuário
            } finally {
    ...
    // Aplicar o filtro de GÊNERO da sidebar sobre os CDs
    ...
    if (loading) {
        return (
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
                <CircularProgress />
                <Typography variant="h6" sx={{ ml: 2 }}>Loading CDs...</Typography>
            </Box>
        );
    }
